...
final public class SignUpVerificationViewController: BaseViewController<VerificationViewModel> {
    var verificationCode = ""
    
    var verificationView = SignUpVerificationView()
    ...
    func makeButtonAction() {
        let continueButtonAction = UIAction { _ in
            Task {
                do {
                    try await self.viewModel?.sendUserNumber(verificationCode: self.verificationCode)
                    
                    let nextVC = SignUpSuccessViewController()
